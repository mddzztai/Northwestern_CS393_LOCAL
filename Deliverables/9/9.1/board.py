from global_variables import BOARD_LENGTH, Board_Exception
import typing
...
class Board:
    ...
    def _check_coordinate(self, coordinate):
        if isinstance(coordinate, tuple):
            if len(coordinate) == 2:
                return
        raise Board_Exception('coordinate not valid')

    def _check_stone_type(self, stone):
        if stone not in ['B', 'W']:
            raise Board_Exception('stone type not valid')