from board import *
import copy
from global_variables import BOARD_LENGTH, Rule_Checker_Exception
...
class rule_checker:
    ...
    def _check_valid_coordinate(self, coordinate: tuple) -> bool: 
        if len(coordinate) == 2 and 0 <= coordinate[0] < BOARD_LENGTH and 0 <= coordinate[1] < BOARD_LENGTH:
            return True
        raise Rule_Checker_Exception('Not valid Coordinate')
    
    def _check_valid_stone_type(self, stone:str) -> bool:
        if stone in ["B", "W"]:
            return True
        raise Rule_Checker_Exception('Not valid Stone Type')

    def _check_valid_maybeStone_type(self, maybeStone:str) -> bool:
        if maybeStone in ["B", "W", " "]:
            return True
        raise Rule_Checker_Exception('Not valid MaybeStone Type')
    
    def check_pass(self, stone:str) -> bool:
        return self._check_valid_stone_type(stone)
    
    def check_move(self, player:str, coordinate:tuple, board_history:list) -> bool:
    ...
    def check_if_move_valid(self, player:str, coordinate:tuple, board_history:list) -> bool:
    ...
    def check_move_not_suicide(self, player:str, coordinate:tuple, B) ->bool:
        B2 = copy.deepcopy(B)
    ...
    def check_if_board_history_valid(self, player:str, board_history:list) -> bool:
    ...
    def check_between_consecqutive_boards(self, board1:list, board2:list, p1b:list, p2b:list, p1w:list, p2w:list):
    ...
    def _check_diff_points(self, p1:list, p2:list):
        stones = set(p1)
        add = []        
    ...
    def _check_empty_board(self, board:list) -> bool:
        for i in range(BOARD_LENGTH):
                for j in range(BOARD_LENGTH):
                    if board[i][j]!=' ': return False
        return True

    def take_out_stones_without_liberty(self, stone:str, B):
        to_remove = []
        for i in range(BOARD_LENGTH):
            for j in range(BOARD_LENGTH):
